...
def replace_leaf(t, old, new):
    ...
    replaced = []
    for b in branches(t):
        replaced = replaced + [replace_leaf(b, old, new)]
...
def balanced(m):
    ...
    False
    """    
    def total_weight(mob):
        if is_weight(mob):
            return size(mob)
        return total_weight(end(left(mob))) + total_weight(end(right(mob)))
...
def make_counter():
    ...
    5
    """
    def counter(arg, record={}):
        if arg not in record:
            record[arg] = 1
        else:
            record[arg] = record[arg] + 1
        return record[arg]
    return counter
...
def make_fib():
    ...
    12
    """
    fib1, fib2, call = 0, 1, 0
    def fib():
        nonlocal fib1, fib2, call
        if call == 0:
            call += 1
            return 0
        elif call == 1:
...
def make_withdraw(balance, password):
    ...
    True
    """
    def withdraw(amount, password_attempt, attempts=[]):
        nonlocal balance
...
def make_joint(withdraw, old_password, new_password):
    ...
    "Your account is locked. Attempts: ['my', 'secret', 'password']"
    """
    withdraw_call = withdraw(0, old_password)
    if type(withdraw_call) == str:
    ...
    def joint_withdraw(amount, attempt):
        ...
        withdraw_call = withdraw(amount, attempt)
        if type(withdraw_call) == str:
            return withdraw_call
